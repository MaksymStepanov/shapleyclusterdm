...
finalstate = "kNEUTRINO"

output_folder = '/home/path/output'
max_iterations = 25
clumpy_output_file = f'spectra_CIRELLI11_EW_NEUTRINO_m{mass}.txt'  # This is the file CLUMPY generates
...
def rename_existing_files(cluster, channel, DMprocess):
    ...
    if os.path.exists(original_file_path):
        if dmprocess == "Ann":
           new_file_name = f"spectraN_{cluster}_{DMprocess}{sigmav}_{channel}_m{mass}.txt"
        elif dmprocess == "Dec":
           new_file_name = f"spectraN_{cluster}_{DMprocess}{tau}_{channel}_m{mass}.txt"
...
def run_clumpy_command(cluster, value, redshift, br_string, DMprocess):
    ...
    command = (
        ...
        f'--gPP_BR={br_string} ' 
        f'--gSIM_FLUX_FLAG_FINALSTATE={finalstate}'
    )
